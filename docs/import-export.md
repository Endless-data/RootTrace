...
在网页中新建一个族谱后，可以进入该族谱的“导入导出”页面上传三份 CSV。这里的 `members.csv` 仍需要填写 `id`，但它只是本次导入文件内部使用的临时编号，用于让亲子关系和婚姻关系引用成员。

导入时系统会忽略 `members.csv` 中的 `family_tree_id`，并把成员写入当前打开的族谱；数据库会自动生成真正的成员主键。因此，单独导入某个新族谱时，`members.csv` 的 `id` 可以从 `1` 开始，不需要接着数据库已有成员 ID。
